...
use gdk::{CommitNode, ConvergenceMetrics, FileThread, ThreadColor};
...
fn bench_thread_color_calculation(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000] {
        group.bench_with_input(
            BenchmarkId::new("batch_calculation", size),
            &size,
            |b, &size| {
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                ...
                b.iter(|| {
                    ...
                    let recent_avg: f64 =
                        recent_commits.clone().sum::<f64>() / recent_commits.count().max(1) as f64;
                    ...
                    black_box(ConvergenceMetrics {
                        ...
                        successful_builds: quality_trend.iter().filter(|&&q| q > 0.7).count()
                            as u32,
...
fn bench_serialization(c: &mut Criterion) {
    ...
    for thread_count in [1, 10, 50, 100] {
        ...
        group.bench_with_input(
            ...
            &commit,
            |b, commit| b.iter(|| black_box(serde_json::to_string(commit).unwrap())),
        );
        ...
        group.bench_with_input(
            ...
            &json,
            |b, json| b.iter(|| black_box(serde_json::from_str::<CommitNode>(json).unwrap())),
        );
    }
...
fn bench_large_collections(c: &mut Criterion) {
    ...
    for size in [100, 500, 1000, 5000] {
        ...
        group.bench_with_input(
            ...
            |b, threads| {
                b.iter(|| {
                    let health_score = threads.values().map(|t| t.functionality_score).sum::<f64>()
...
fn bench_memory_patterns(c: &mut Criterion) {
    ...
    group.bench_function("uuid_generation", |b| b.iter(|| black_box(Uuid::new_v4())));
