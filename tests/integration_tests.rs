...
use gdk::{CommitNode, GdkResult, GitWorkflow, ThreadColor};
use std::fs;
...
async fn setup_test_repo() -> GdkResult<(TempDir, GitWorkflowManager)> {
    ...
    let repo = Repository::init(repo_path)
        .map_err(|e| gdk::GdkError::git_error("initializing repository", e))?;
...
"#,
    ...
    let sig = Signature::now("Test User", "test@example.com")
        .map_err(|e| gdk::GdkError::git_error("creating signature", e))?;

    let mut index = repo
        .index()
        .map_err(|e| gdk::GdkError::git_error("getting index", e))?;

    index
        .add_path(std::path::Path::new("src/lib.rs"))
        .map_err(|e| gdk::GdkError::git_error("adding file to index", e))?;
    index
    ...
    let tree_id = index
        .write_tree()
        .map_err(|e| gdk::GdkError::git_error("writing tree", e))?;

    let tree = repo
        .find_tree(tree_id)
        .map_err(|e| gdk::GdkError::git_error("finding tree", e))?;

    repo.commit(Some("HEAD"), &sig, &sig, "Initial commit", &tree, &[])
        .map_err(|e| gdk::GdkError::git_error("creating initial commit", e))?;
...
async fn test_thread_color_scoring() {
    ...
    assert_eq!(
        ThreadColor::from_scores(0.8, 0.8, 0.8, 0.8),
        ThreadColor::LightGreen
    );
    assert_eq!(
        ThreadColor::from_scores(0.6, 0.6, 0.6, 0.6),
        ThreadColor::Yellow
    );
    assert_eq!(
        ThreadColor::from_scores(0.4, 0.4, 0.4, 0.4),
        ThreadColor::Orange
    );
    assert_eq!(
        ThreadColor::from_scores(0.1, 0.1, 0.1, 0.1),
        ThreadColor::Red
    );

    // Test mixed scores
    assert_eq!(
        ThreadColor::from_scores(1.0, 0.8, 0.6, 0.4),
        ThreadColor::LightGreen
    ); // avg = 0.7
    assert_eq!(
        ThreadColor::from_scores(0.9, 0.9, 0.9, 1.0),
        ThreadColor::Green
    ); // avg = 0.925
}
...
async fn test_error_handling() -> GdkResult<()> {
    ...
    let git_error = gdk::GdkError::git_error("test operation", git2::Error::from_str("test error"));
    assert_eq!(git_error.category(), "git");
    assert!(!git_error.is_recoverable()); // Generic git errors not recoverable

    let validation_error =
        gdk::GdkError::validation_error("lint", "style_check", "Missing documentation");
    ...
    let convergence_error =
        gdk::GdkError::convergence_error("Failed to meet threshold", 10, 0.7, 0.8);
...
async fn test_serialization() -> GdkResult<()> {
    ...
    let json = serde_json::to_string(&commit).map_err(|e| gdk::GdkError::SerializationError {
    ...
    let deserialized: CommitNode =
        serde_json::from_str(&json).map_err(|e| gdk::GdkError::SerializationError {
            format: "JSON".to_string(),
